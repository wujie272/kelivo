...
void main() {
  ...
  Future<File> writeLegacyZip({
    ...
    for (final entry in assets.entries) {
      final assetFile = File('${root.path}/asset_${entry.key.replaceAll('/', '_')}');
  ...
  test('new chats.json parts round-trip without reintroducing markers', () async {
  ...
  test('new chats.json with literal [image:] text in parts is not promoted', () async {
    const literal = 'mention\n[image:https://example.com/literal.png]\nas text';
    final original = ChatMessage(
      id: 'm1',
      role: 'user',
      conversationId: 'c1',
  ...
  test('recomputeAttachmentAvailability flips local missing to available', () {
    ...
    final remote = ImagePart(uri: 'https://example.com/a.png', unavailable: false);
  ...
  test('local attachment becomes available after chats+files restore', () async {
